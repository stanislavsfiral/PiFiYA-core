...
document.getElementById('togglePhotonsBtn').addEventListener('click', () => {
    photonsActive = !photonsActive;
    const btn = document.getElementById('togglePhotonsBtn');
    if (photonsActive) {
        btn.style.background = '#00ffcc';
        btn.style.color = '#0b0f19';
        btn.textContent = '⚡ Импульсы: ВКЛ';
        spawnPhotonStream();
    } else {
        btn.style.background = '#1f2d4a';
        btn.style.color = '#00ffcc';
        btn.textContent = '⚡ Импульсы: ВЫКЛ';
        photonParticles.forEach(p => photonGroup.remove(p.mesh));
        photonParticles = [];
        requestRender();
    }
});
...
function updatePhotons() {
    if (!photonsActive) return;
    photonParticles.forEach(p => {
        p.progress += p.speed;
        if (p.progress > 1) p.progress = 0;
        p.mesh.position.lerpVectors(p.start, p.end, p.progress);
    });
    if (photonParticles.length > 0) requestRender();
}
...
function spawnPhotonStream() {
    ...
    photonParticles = [];
    graph.edges.forEach(edge => {
        const fNode = getNode(edge.from);
        const tNode = getNode(edge.to);
        const fGroup = meshMap.get(edge.from);
        const tGroup = meshMap.get(edge.to);
        if (!fNode || !tNode || !fGroup || !tGroup) return;

        const startWorld = new THREE.Vector3();
        const endWorld = new THREE.Vector3();
        fGroup.localToWorld(fNode.exitPoint.clone(startWorld));
        tGroup.localToWorld(tNode.entryPoint.clone(endWorld));

        const pGeo = new THREE.SphereGeometry(2.5, 8, 8);
        const pMat = new THREE.MeshBasicMaterial({ color: 0x00ffcc, transparent: true, opacity: 0.9 });
...
document.getElementById('addNodeBtn').addEventListener('click', () => {
    ...
    const node = addNode(document.getElementById('modeSelectEditor').value, (count % 5 - 2) * 80, Math.floor(count / 5) * 80, 0, { N: 5, target_len: 1000, scale: 1.0, angles: [180,0,90] });
    updateNodeVisual(node);
    updateEdges();
    updateStats();
...
document.getElementById('buildFractalBtn').addEventListener('click', buildFractalComposition);
...
document.getElementById('simulateBtn').addEventListener('click', () => {
    ...
    if (photonsActive) spawnPhotonStream();
...
document.getElementById('saveSpectrumBtn').addEventListener('click', () => {
    if (!lastComputedSpectrum) return alert('Сначала выполните симуляцию!');
    const data = JSON.stringify({ timestamp: new Date().toISOString(), spectrum: lastComputedSpectrum }, null, 2);
    const url = URL.createObjectURL(new Blob([data], { type: 'application/json' }));
    const a = document.createElement('a'); a.href = url; a.download = 'sfiral_spectrum.json';
...
document.getElementById('exportPythonBtn').addEventListener('click', () => {
    ...
    const py = `import numpy as np\nnodes = ${JSON.stringify(graph.nodes, null, 4)}\nedges = ${JSON.stringify(graph.edges, null, 4)}\nprint(f"Экспортировано сфиралей: {len(nodes)}")\n`;
    const consoleEl = document.getElementById('console');
    consoleEl.style.display = 'block';
    consoleEl.innerHTML = `<div class="line success">🐍 Python-код:</div><pre style="color:#00ffaa; user-select:all;">${py}</pre>`;
});
...
function init() {
    ...
    function ambientLoop() {
        if (photonsActive || walshEngine.active) requestRender();
