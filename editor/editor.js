...
// --- ИНТЕГРАЦИЯ evaluateChainBridge В ХРОНОКВАНТЫ ---
function updateChronoQuantumLinks() {
    ...
    for (let i = 0; i < graph.nodes.length; i++) {
        for (let j = i + 1; j < graph.nodes.length; j++) {
            ...
            // Вызываем расчет моста суперпозиции через общий виток из движка Уолша
            ...
            if (bridge.isBridge) {
                ...
                activeChronoLinks.push({
                    ...
                    chirality: bridge.chirality, // Учитываем знак хиральности стыка (+1 / -1)
...
function updateEdges() {
    ...
    chronoLinks.forEach(link => {
        ...
        // Цвет линии хронокванта меняется в зависимости от хиральности общего витка (+1 синий / -1 оранжевый)
...
// --- ОДНОРАЗОВЫЙ РАСЧЕТ УОЛША (X, Y, Z) ---
document.getElementById('toggleWalshBtn').addEventListener('click', () => {
    if (graph.nodes.length === 0) { alert('Нет узлов для расчета'); return; }
    
    const consoleEl = document.getElementById('console');
    consoleEl.style.display = 'block';
    consoleEl.innerHTML = '<div class="line">🌀 Вычисление троичного пространственного спектра Уолша (X, Y, Z)...</div>';
    ...
    consoleEl.innerHTML = `
        <div class="line success">✅ Расчет по Уолшу (X,Y,Z) завершен!</div>
        <div class="line">📊 Активных фаз (+1): ${positiveCount} | Нейтральных (0): ${zeroCount} | Инверсных (-1): ${negativeCount}</div>
    `;
    requestRender();
});
...
// --- ОДНОРАЗОВЫЙ РЕЗОНАНСНЫЙ АНАЛИЗ + ФУРЬЕ ---
document.getElementById('simulateBtn').addEventListener('click', () => {
    if (graph.nodes.length === 0) { alert('Нет узлов'); return; }
    const consoleEl = document.getElementById('console');
    consoleEl.style.display = 'block';
    consoleEl.innerHTML = '<div class="line">⚙️ Выполнение разового резонансного анализа и Фурье...</div>';
    ...
    consoleEl.innerHTML = `
        <div class="line success">✅ Резонанс + Фурье успешно просчитаны!</div>
        <div class="line">📊 Итоговая энергия системы: ${totalEnergy.toFixed(4)} | Узлов в балансе: ${graph.nodes.length}</div>
    `;
});
