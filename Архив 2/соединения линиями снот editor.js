...
function autoConnectStuckNodes() {
    // Функция переведена в пассивный режим подсказок: 
    // она больше не двигает узлы автоматически, предотвращая любые искажения геометрии.
...
function updateSelectionHighlights() {
    ...
    if (selectedNodes.length === 1) {
        ...
        if (targetGroup) {
            // Привязываем контроллер строго к корневой группе узла, находящейся в сцене
...
class UIManager {
    ...
    setupQuantumGates() {
        document.getElementById('gateHadamard')?.addEventListener('click', () => {
            if (selectedNodes.length === 0) { alert('Выберите сфираль для применения вентиля H'); return; }
            saveState();
            selectedNodes.forEach(id => {
                const node = getNode(id);
                if (node) {
                    node.quantumState.intensity = 0.5;
        ...
        document.getElementById('gateCNOT')?.addEventListener('click', () => {
            ...
            if (edge) { 
                ...
                alert('Связь между этими сфиралями уже установлена'); 
            }
        });
    }
